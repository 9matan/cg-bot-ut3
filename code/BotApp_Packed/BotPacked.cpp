...
namespace ut3 {namespace bot {class CMCTSBot_v1 : public CBotBase{public:static SVec2 FindTurn(game::SGameState const& gameState, int const myPlayer, bool const debugEnabled = false);public:char const* GetBotName() const override { return "CMCTSBot_v1"; }protected:SVec2 FindTurn(game::SGameState const& gameState) override;};}}
void ReadData(ut3::SInputData& inputData){std::cin >> inputData.m_oppTurnY >> inputData.m_oppTurnX;int validActionCount;scanf("%i", &validActionCount);for (int i = 0; i < validActionCount; ++i){int x, y;scanf("%i%i", &x, &y);}}void WriteData(ut3::SOutputData const& outputData){std::cout << outputData.m_turnY << " " << outputData.m_turnX << "\n";}int main(){int const randomSeed = mimax::common::UpdateRandomSeed();std::cerr << "Random seed: " << randomSeed << "\n";std::cerr << "Hardware concurrency: " << std::thread::hardware_concurrency() << "\n";ut3::SInputData inData;ReadData(inData);ut3::bot::CMCTSBot_v1 bot;bot.SetDebugIsEnabled(false);WriteData(bot.FirstUpdate(inData));while (true){ReadData(inData);{PROFILE_TIME("Update");WriteData(bot.Update(inData));}}return 0;}
...
namespace mimax {namespace dma {template<typename TState, typename TMove, typename TMovesContainer, typename TResolver>class CMctsBase{public:CMctsBase(TState const& rootSate, TResolver const& resolver, unsigned long long const randomSeed, float const explorationParam = 1.4142f): m_resolver(resolver), m_randomEngine(randomSeed), m_explorationParam(explorationParam)
#if MIMAX_MCTS_DEBUG
, m_currentDepth(0)
#endif 
{m_root = SNode();m_root.m_state = rootSate;bool const successful = Expanse(&m_root);assert(successful);}void Evaluate(){MakeIteration();}TMove GetCurrentResult() const{return std::max_element(m_root.m_children.begin(), m_root.m_children.end(),[](SNode const& lhs, SNode const& rhs){return lhs.m_simulationsCount < rhs.m_simulationsCount;})->m_move;}SMctsNodeEvalResult<TMove> GetRootEvalResult() const{SMctsNodeEvalResult<TMove> evalResult;evalResult.m_childrenInfo.resize(m_root.m_children.size());for (size_t i = 0; i < m_root.m_children.size(); ++i){evalResult.m_childrenInfo[i].m_move = m_root.m_children[i].m_move;evalResult.m_childrenInfo[i].m_simulationsCount = m_root.m_children[i].m_simulationsCount;}return std::move(evalResult);}
#if MIMAX_MCTS_DEBUG
inline SMctsDebugInfo const& GetDebugInfo() const { return m_debugInfo; }
#endif 
private:struct SNode{SNode(): m_simulationsCount(0), m_score(0), m_uctScore(0.0f), m_parent(nullptr), m_unvisitedChildrenCount(0){}inline bool IsVisited() const { return m_simulationsCount > 0; }inline bool HasChildren() const { return !m_children.empty(); }inline bool HasUnvisitedChildren() const { return m_unvisitedChildrenCount > 0; }std::vector<SNode> m_children;unsigned int m_simulationsCount;float m_score;float m_uctScore;SNode* m_parent;TState m_state;TMove m_move;unsigned short m_unvisitedChildrenCount;};private:std::mt19937_64 m_randomEngine;TResolver m_resolver;SNode m_root;float m_explorationParam;TMovesContainer m_movesBuffer;
#if MIMAX_MCTS_DEBUG
SMctsDebugInfo m_debugInfo;size_t m_currentDepth;
#endif 
private:bool Expanse(SNode* node){m_resolver.GetPossibleMoves(node->m_state, m_movesBuffer);std::shuffle(m_movesBuffer.begin(), m_movesBuffer.end(), m_randomEngine);node->m_children.resize(m_movesBuffer.size());auto curChild = node->m_children.begin();for (auto const move : m_movesBuffer){curChild->m_parent = node;curChild->m_move = move;++curChild;}node->m_unvisitedChildrenCount = (unsigned short)node->m_children.size();
#if MIMAX_MCTS_DEBUG
m_debugInfo.ExpanseNode(m_currentDepth, node->m_children.size());
#endif 
return !m_movesBuffer.empty();}void MakeIteration(){SNode* curNode = SelectChildNode(&m_root);
#if MIMAX_MCTS_DEBUG
m_currentDepth = 1;
#endif 
...
#endif 
return m_resolver.Playout(node->m_state);}inline void UpdateStatistics(SNode* node, float const score){++node->m_simulationsCount;node->m_score += score;}inline SNode* GetFirstUnvisitedChild(SNode* node){auto iter = std::find_if(node->m_children.begin(), node->m_children.end(),[](SNode const& child){return !child.IsVisited();});assert(iter != node->m_children.end());return &(*iter);}inline SNode* GetBestNodeByUCT(SNode* node){for (auto& child : node->m_children){child.m_uctScore = CalculateUCTScore(&child);}auto iter = std::max_element(node->m_children.begin(), node->m_children.end(),[](SNode const& lhs, SNode const& rhs){return lhs.m_uctScore < rhs.m_uctScore;});return &(*iter);}inline float CalculateUCTScore(SNode const* node){auto const parentSimCount = node->m_parent->m_simulationsCount;return  1.0f - (float)node->m_score / (float)node->m_simulationsCount+ m_explorationParam * (float)sqrt(log(parentSimCount) / node->m_simulationsCount);}};} } 
...
namespace ut3 {namespace bot {namespace {class CUT3MctsResolver{public:inline static float CalculateScore(game::SGameState const& state, int myPlayer){int const gameWinner = GAME_STATE_GET_GAME_WINNER(state);if (gameWinner != GAME_STATE_ELEMENT_EMPTY){if (gameWinner == GAME_STATE_ELEMENT_DRAW) return 0.5f;else if (gameWinner == GAME_STATE_PLAYER_INDEX_TO_PLAYER_ELEMENT(myPlayer)) return 1.0f;else return 0.0f;}auto const globalBlock = GAME_STATE_GET_GLOBAL_BLOCK(state);int const oppPlayer = ((myPlayer + 1) & 1);int const myBlocksCnt = (int)GAME_STATE_BLOCK_COUNT_PLAYER_ELEMENTS(globalBlock, myPlayer);int const oppBlocksCnt = (int)GAME_STATE_BLOCK_COUNT_PLAYER_ELEMENTS(globalBlock, oppPlayer);return (myBlocksCnt == oppBlocksCnt? 0.5f: (myBlocksCnt > oppBlocksCnt ? 1.0f : 0.0f));}public:void GetPossibleMoves(game::SGameState const& state, game::Turns& movesOut){game::CollectPossibleTurns(state, movesOut);}void MakeMove(game::SGameState& state, SVec2 const move){game::MakeTurn(state, move[0], move[1]);}float Playout(game::SGameState const& state){int myPlayer = GAME_STATE_GET_PLAYER(state);game::SGameState curState = state;game::Turns moves;for (size_t depth = 0; depth < 8; ++depth){game::CollectPossibleTurns(curState, moves);if (moves.empty()) break;auto const move = *mimax::common::GetRandomItem(moves.begin(), moves.end());game::MakeTurn(curState, move[0], move[1]);}return CalculateScore(curState, myPlayer);}};using CUT3Mcts = mimax::dma::CMctsBase<game::SGameState, SVec2, game::Turns, CUT3MctsResolver>;using CUT3MctsTask = mimax::dma::CMctsTask<CUT3Mcts>;static SVec2 CalculateBestMove(std::vector<CUT3Mcts> const& mctsAlgos){mimax::dma::SMctsNodeEvalResult<SVec2> combinedResult;for (auto const& algo : mctsAlgos){mimax::dma::mcts_utility::CombineEvalResults(combinedResult, algo.GetRootEvalResult());}return mimax::dma::mcts_utility::GetBestMove(combinedResult);}} SVec2 CMCTSBot_v1::FindTurn(game::SGameState const& gameState){return FindTurn(gameState, m_myPlayer, m_isDebugEnabled);}SVec2 CMCTSBot_v1::FindTurn(game::SGameState const& gameState, int const /*myPlayer*/, bool const debugEnabled){if (GAME_STATE_ELEMENTS_COUNT(gameState) == 0){return { 4, 4 };}std::vector<CUT3Mcts> mctsAlgos;std::vector<CUT3MctsTask> mctsTasks;std::vector<mimax::mt::ITask*> tasksToRun;auto const hardwareConcurrency = std::thread::hardware_concurrency();mctsAlgos.reserve(hardwareConcurrency);mctsTasks.reserve(hardwareConcurrency);tasksToRun.reserve(hardwareConcurrency);std::mt19937_64 randEngine(time(NULL));CUT3MctsResolver resolver;for (size_t i = 0; i < hardwareConcurrency; ++i){auto& mcts = mctsAlgos.emplace_back(gameState, resolver, randEngine(), 1.41f);auto& mctsTask = mctsTasks.emplace_back(&mcts);tasksToRun.push_back(&mctsTask);}mimax::mt::CTasksRunner().RunTasksAndWait(tasksToRun, std::chrono::milliseconds(90));if (debugEnabled){
#if MIMAX_MCTS_DEBUG
