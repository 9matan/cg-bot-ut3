...
namespace ut3 {namespace bot {struct SBotBehaviorInitParams;class IBotBehavior{public:virtual ~IBotBehavior() = default;virtual void Initialize(game::SGameState const& gameState, SBotBehaviorInitParams const& initParams) = 0;virtual SVec2 FindTurn(game::SGameState const& gameState) = 0;virtual void Shutdown() = 0;};} } 
namespace ut3 {namespace bot {class CBotBehaviorBase : public IBotBehavior{};} } 
namespace ut3 {namespace bot {class CBbCustom : public CBotBehaviorBase{public:using FindTurnFunc = std::function<SVec2(game::SGameState const& gameState)>;CBbCustom(FindTurnFunc findTurnFunc);void Initialize(game::SGameState const& /*gameState*/, SBotBehaviorInitParams const& /*initParams*/) override {}SVec2 FindTurn(game::SGameState const& gameState) override;void Shutdown() override {}private:FindTurnFunc m_findTurnFunc;};} } 
namespace ut3{struct SInputData{int m_oppTurnX;int m_oppTurnY;};};
namespace ut3{struct SOutputData{int m_turnX;int m_turnY;};};
namespace ut3 {namespace bot {class IBot{public:virtual ~IBot() = default;virtual SOutputData FirstUpdate(SInputData initData) = 0;virtual SOutputData Update(SInputData turnData) = 0;virtual void SetDebugIsEnabled(bool const enableDebug) = 0;virtual char const* GetBotName() const = 0;virtual void Reset() = 0;};} } 
namespace ut3 {namespace bot {class IBotBehavior;class CBot : public IBot{public:CBot(std::unique_ptr<IBotBehavior> botBehavior, char const* botName);~CBot() override;SOutputData FirstUpdate(SInputData initData) override;SOutputData Update(SInputData turnData) override;void SetDebugIsEnabled(bool const enableDebug) override { m_isDebugEnabled = enableDebug; }void Reset() override;char const* GetBotName() const override { return m_botName.c_str(); }inline void SetTimeout(std::chrono::milliseconds const timeout) { m_timeout = timeout; }template<typename TBb>inline TBb* GetBb() { return static_cast<TBb*>(m_botBehavior.get()); }private:void InitializeBB();SVec2 FindAndMakeTurn();private:game::SGameState m_gameState;std::string const m_botName;std::unique_ptr<IBotBehavior> m_botBehavior;std::chrono::milliseconds m_timeout;bool m_isDebugEnabled;};} } 
namespace ut3 {namespace bot {CBot* CreateCustomBot(CBbCustom::FindTurnFunc func, char* const botName = "Custom");CBot* CreateMctsBot(float const explorationParam, size_t const maxDepth, char* const botName = "Mcts");CBot* CreateMinimaxBot(char* const botName = "Minimax");CBot* CreateRandomBot(char* const botName = "Random");} } 
...
#endif
ut3::SInputData inData;ReadData(inData);auto bot = ut3::bot::CreateMctsBot(0.35f, 30);
#if RELEASE_BOT
bot->SetDebugIsEnabled(false);
#else
bot->SetDebugIsEnabled(true);
#endif
bot->SetTimeout(std::chrono::milliseconds(90));WriteData(bot->FirstUpdate(inData));while (true){ReadData(inData);{PROFILE_TIME("Update");WriteData(bot->Update(inData));}}return 0;}
...
namespace ut3 {namespace bot {struct SBotBehaviorInitParams{std::chrono::milliseconds m_timeout = std::chrono::milliseconds(90);size_t m_randomSeed = 0;bool m_isDebugEnabled = false;};} } 
...
#endif 
private:std::vector<CUT3Mcts> m_mctsAlgos;SBotBehaviorInitParams m_params;float m_explorationParam;size_t m_maxDepth;};} } 
namespace ut3 {namespace bot {class CBbMinimax : public CBotBehaviorBase{public:void Initialize(game::SGameState const& gameState, SBotBehaviorInitParams const& initParams) override;SVec2 FindTurn(game::SGameState const& gameState) override;void Shutdown() override {}private:SBotBehaviorInitParams m_params;};} } 
namespace ut3 {namespace bot {class CBbRandom : public CBotBehaviorBase{public:void Initialize(game::SGameState const& /*gameState*/, SBotBehaviorInitParams const& /*initParams*/) override {}SVec2 FindTurn(game::SGameState const& gameState) override;void Shutdown() override {}};} } 
namespace ut3 {namespace bot {CBot* CreateCustomBot(CBbCustom::FindTurnFunc func, char* const botName){auto bb = std::make_unique<CBbCustom>(func);return new CBot(std::move(bb), botName);}CBot* CreateMctsBot(float const explorationParam, size_t const maxDepth, char* const botName){auto bb = std::make_unique<CBbMcts>(explorationParam, maxDepth);return new CBot(std::move(bb), botName);}CBot* CreateMinimaxBot(char* const botName){auto bb = std::make_unique<CBbMinimax>();return new CBot(std::move(bb), botName);}CBot* CreateRandomBot(char* const botName){auto bb = std::make_unique<CBbRandom>();return new CBot(std::move(bb), botName);}} } 
...
namespace ut3 {namespace bot {using namespace std;CBot::CBot(unique_ptr<IBotBehavior> botBehavior, char const* botName): m_botBehavior(move(botBehavior)), m_timeout(90), m_botName(botName), m_isDebugEnabled(false){}CBot::~CBot(){}SOutputData CBot::FirstUpdate(SInputData initData){if (initData.m_oppTurnX >= 0){game::MakeTurn(m_gameState, initData.m_oppTurnX, initData.m_oppTurnY);}InitializeBB();auto const turn = FindAndMakeTurn();return { turn[0], turn[1] };}SOutputData CBot::Update(SInputData turnData){game::MakeTurn(m_gameState, turnData.m_oppTurnX, turnData.m_oppTurnY);if (m_isDebugEnabled){game::SGameStateView(m_gameState).Print();}auto const turn = FindAndMakeTurn();return { turn[0], turn[1] };}SVec2 CBot::FindAndMakeTurn(){auto const turn = m_botBehavior->FindTurn(m_gameState);game::MakeTurn(m_gameState, turn[0], turn[1]);return turn;}void CBot::Reset(){m_botBehavior->Shutdown();m_gameState = game::SGameState();}void CBot::InitializeBB(){SBotBehaviorInitParams params;params.m_isDebugEnabled = m_isDebugEnabled;params.m_timeout = m_timeout;params.m_randomSeed = (size_t)time(time_t(NULL)); m_botBehavior->Initialize(m_gameState, params);}} } 
...
namespace ut3 {namespace bot {CBbCustom::CBbCustom(FindTurnFunc findTurnFunc): m_findTurnFunc(findTurnFunc){}SVec2 CBbCustom::FindTurn(game::SGameState const& gameState){return m_findTurnFunc(gameState);}} } 
namespace ut3 {namespace bot {SVec2 CBbRandom::FindTurn(game::SGameState const& gameState){mimax::common::UpdateRandomSeed();game::Turns turns;game::CollectPossibleTurns(gameState, turns);return *mimax::common::GetRandomItem(turns.begin(), turns.end());}} } 
...
using namespace std::chrono_literals;namespace ut3 {namespace bot {namespace {class CUT3MinimaxResolver{public:CUT3MinimaxResolver(int const myPlayer): m_myPlayer(myPlayer){}float EvaluateState(game::SGameState const& state){int const gameWinner = GAME_STATE_GET_GAME_WINNER(state);if (gameWinner != GAME_STATE_ELEMENT_EMPTY){if (gameWinner == GAME_STATE_ELEMENT_DRAW) return 0.0f;float const depthScore = GAME_STATE_ELEMENTS_COUNT(state) * 0.01f;if (gameWinner == GAME_STATE_PLAYER_INDEX_TO_PLAYER_ELEMENT(m_myPlayer)) return 10.0f - depthScore;return -(10.0f - depthScore);}auto const globalBlock = GAME_STATE_GET_GLOBAL_BLOCK(state);int const oppPlayer = ((m_myPlayer + 1) & 1);int const myBlocksCnt = (int)GAME_STATE_BLOCK_COUNT_PLAYER_ELEMENTS(globalBlock, m_myPlayer);int const oppBlocksCnt = (int)GAME_STATE_BLOCK_COUNT_PLAYER_ELEMENTS(globalBlock, oppPlayer);return (float)(myBlocksCnt - oppBlocksCnt);}void GetPossibleMoves(game::Turns& turnsOut, game::SGameState const& state){game::CollectPossibleTurns(state, turnsOut);mimax::common::RandomShuffle(turnsOut.begin(), turnsOut.end());}void MakeMove(game::SGameState& state, SVec2 move){game::MakeTurn(state, move[0], move[1]);}private:int m_myPlayer;};using CUT3MinimaxAlgo = mimax::dma::CMinimaxBase<game::SGameState, SVec2, game::Turns, CUT3MinimaxResolver>;using CMinimaxTask = mimax::dma::CMinimaxTask<CUT3MinimaxAlgo>;} void CBbMinimax::Initialize(game::SGameState const& /*gameState*/, SBotBehaviorInitParams const& initParams){m_params = initParams;}SVec2 CBbMinimax::FindTurn(game::SGameState const& gameState){if (GAME_STATE_ELEMENTS_COUNT(gameState) == 0){return { 4, 4 };}auto globalBlock = GAME_STATE_GET_GLOBAL_BLOCK(gameState);size_t const playedBlocksCount =GAME_STATE_BLOCK_COUNT_PLAYER_ELEMENTS(globalBlock, 0)+ GAME_STATE_BLOCK_COUNT_PLAYER_ELEMENTS(globalBlock, 1)+ GAME_STATE_BLOCK_COUNT_DRAW_ELEMENTS(globalBlock);float const minScoreValue = (playedBlocksCount >= 6 ? (-1.0f) : (-2.0f));std::vector<CUT3MinimaxAlgo> minimaxAlgos;std::vector<CMinimaxTask> minimaxTasks;std::vector<mimax::mt::ITask*> tasksToRun;CUT3MinimaxResolver resolver(GAME_STATE_GET_PLAYER(gameState));CUT3MinimaxAlgo::SConfig config;config.m_minValue = minScoreValue;config.m_maxValue = 10.0f;config.m_epsilon = 0.001f;auto const hardwareConcurrency = std::thread::hardware_concurrency();minimaxAlgos.reserve(hardwareConcurrency);minimaxTasks.reserve(hardwareConcurrency);tasksToRun.reserve(hardwareConcurrency);size_t const step = (hardwareConcurrency <= 4) ? 2 : 1;size_t constexpr minDepth = 6;for (size_t depth = minDepth; depth < minDepth + step * hardwareConcurrency; depth += step){config.m_maxDepth = depth;auto& minimaxAlgo = minimaxAlgos.emplace_back(resolver, config);auto& minimaxTask = minimaxTasks.emplace_back(&minimaxAlgo, gameState);tasksToRun.push_back(&minimaxTask);}mimax::mt::CTasksRunner().RunTasksAndWait(tasksToRun, m_params.m_timeout);SVec2 turn = { -1, -1 };for (int i = (int)minimaxTasks.size() - 1; i >= 0; --i){auto& move = minimaxTasks[i].GetResult();if (move.has_value()){turn = move.value();
#if MIMAX_MINIMAX_DEBUG
if (m_params.m_isDebugEnabled){std::cerr << minimaxAlgos[i].GetDebugInfo();}
#endif 
...
namespace ut3 {namespace game {using FindTurnFunc = std::function<SVec2(SGameState const& gameState)>;SGameState PlayGame(std::array<FindTurnFunc, 2> bots, SGameState gameState = SGameState());SGameState PlayRandomGame(SGameState gameState, size_t const maxDepth);}}
...
#endif 
return CalculateBestMove(m_mctsAlgos);}void CBbMcts::Shutdown(){m_mctsAlgos.clear();}} } 
...
namespace mimax {namespace common {template<typename TElem>void EraseSwapWithLast(std::vector<TElem>& v, TElem const& elem){auto iter = std::find(v.begin(), v.end(), elem);if (iter == v.end()){return;}std::swap(*iter, v.back());v.pop_back();}}}
namespace ut3 {namespace game {namespace {char WORLD_POS_TO_LOCAL[9] = { 0, 1, 2, 0, 1, 2, 0, 1, 2 };} SGameState PlayGame(std::array<FindTurnFunc, 2> bots, SGameState gameState){int currentBotIndex = 0;while (GAME_STATE_GET_GAME_WINNER(gameState) == GAME_STATE_ELEMENT_EMPTY){SVec2 const turn = bots[currentBotIndex](gameState);Turns turns;CollectPossibleTurns(gameState, turns);assert(std::find(turns.begin(), turns.end(), turn) != turns.end());MakeTurn(gameState, turn[0], turn[1]);currentBotIndex = ((currentBotIndex + 1) & 1);}return gameState;}SGameState PlayRandomGame(SGameState gameState, size_t const maxDepth){assert(GAME_STATE_IS_VALID_LAST_OPP_TURN(gameState));using BlockTurns = mimax::common::CStaticVector<SVec2, 9>;BlockTurns availableTurns[9];size_t totalTurnsCount = 0;for (size_t blockX = 0; blockX < 3; ++blockX){for (size_t blockY = 0; blockY < 3; ++blockY){size_t const blockIndex = GAME_BLOCK_STATE_POS_TO_INDEX(blockX, blockY);if (GAME_STATE_GET_BLOCK_STATUS(gameState, blockIndex) == GAME_STATE_ELEMENT_EMPTY){auto const block = GetBlockState(gameState, blockIndex);for (size_t x = 0; x < 3; ++x){for (size_t y = 0; y < 3; ++y){if (GAME_BLOCK_STATE_GET_ELEMENT_BY_POS(block, x, y) == GAME_STATE_ELEMENT_EMPTY){availableTurns[blockIndex].push_back({ x + 3 * blockX, y + 3 * blockY });++totalTurnsCount;}}}}}}SVec2 lastTurn(GAME_STATE_GET_LAST_OPP_TURN_X(gameState), GAME_STATE_GET_LAST_OPP_TURN_Y(gameState));SVec2 turn;for (size_t i = 0; i < maxDepth && GAME_STATE_GET_GAME_WINNER(gameState) == GAME_STATE_ELEMENT_EMPTY; ++i){size_t blockIndex = GAME_BLOCK_STATE_POS_TO_INDEX(WORLD_POS_TO_LOCAL[lastTurn[0]], WORLD_POS_TO_LOCAL[lastTurn[1]]);size_t turnIndex;if (GAME_STATE_GET_BLOCK_STATUS(gameState, blockIndex) == GAME_STATE_ELEMENT_EMPTY){turnIndex = mimax::common::GetRandomSizeT(0, availableTurns[blockIndex].size());}else{turnIndex = mimax::common::GetRandomSizeT(0, totalTurnsCount);blockIndex = 0;while (turnIndex >= availableTurns[blockIndex].size()){turnIndex -= availableTurns[blockIndex].size();++blockIndex;}}turn = availableTurns[blockIndex][turnIndex];std::swap(availableTurns[blockIndex][turnIndex], availableTurns[blockIndex].back());availableTurns[blockIndex].pop_back();--totalTurnsCount;MakeTurn(gameState, turn[0], turn[1]);lastTurn = turn;if (GAME_STATE_GET_BLOCK_STATUS(gameState, blockIndex) != GAME_STATE_ELEMENT_EMPTY){totalTurnsCount -= availableTurns[blockIndex].size();availableTurns[blockIndex].clear();}}return gameState;}}}
...
#include <time.h>
namespace mimax {namespace common {unsigned int GenerateRandomSeed(){return (unsigned int)time(nullptr);}unsigned int UpdateRandomSeed(unsigned int seed){if (seed == 0)seed = GenerateRandomSeed();srand(seed);return seed;}unsigned int GetRandomUInt32(unsigned int const minVal, unsigned int const maxVal){assert(minVal < maxVal);unsigned int const range = maxVal - minVal;return rand() % range + minVal;}size_t GetRandomSizeT(size_t const minVal, size_t const maxVal){assert(minVal < maxVal);size_t const range = maxVal - minVal;return rand() % range + minVal;}}}
