...
namespace ut3{class CMinimaxBot_v1{public:static SVec2 FindTurn(game::SGameState const& gameState, int const myPlayer);public:CMinimaxBot_v1(SInputData initData);SOutputData FirstUpdate(SInputData initData);SOutputData Update(SInputData turnData);private:int m_myPlayer;game::SGameState m_gameState;};}

#include <iostream>
void ReadData(ut3::SInputData& inputData){std::cin >> inputData.m_oppTurnY >> inputData.m_oppTurnX;int validActionCount;scanf("%i", &validActionCount);for (int i = 0; i < validActionCount; ++i){int x, y;scanf("%i%i", &x, &y);}}void WriteData(ut3::SOutputData const& outputData){std::cout << outputData.m_turnY << " " << outputData.m_turnX << "\n";}int main(){ut3::SInputData inData;ReadData(inData);ut3::CMinimaxBot_v1 bot(inData);WriteData(bot.FirstUpdate(inData));while (true){ReadData(inData);{PROFILE_TIME("Update");WriteData(bot.Update(inData));}}return 0;}
