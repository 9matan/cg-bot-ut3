...
namespace ut3 {namespace bot {class CBotBase : public IBot{public:CBotBase(char const* botName);SOutputData FirstUpdate(SInputData initData) override;SOutputData Update(SInputData turnData) override;void SetDebugIsEnabled(bool const enableDebug) override { m_isDebugEnabled = enableDebug; }void Reset() override;char const* GetBotName() const override { return m_botName.c_str(); }void SetState(game::SGameState const& gameState);inline void SetTimeout(std::chrono::milliseconds const timeout) { m_timeout = timeout; }protected:virtual SVec2 FindTurn(game::SGameState const& gameState) = 0;protected:std::string const m_botName;game::SGameState m_gameState;int m_myPlayer = -1;bool m_isDebugEnabled = false;std::chrono::milliseconds m_timeout;};}}
...
#endif
ut3::SInputData inData;ReadData(inData);ut3::bot::CMCTSBot_v1 bot(0.35f, 60);
#if RELEASE_BOT
...
namespace ut3 {namespace bot {CBotBase::CBotBase(char const* botName): m_timeout(90), m_botName(botName){Reset();}SOutputData CBotBase::FirstUpdate(SInputData initData){if (initData.m_oppTurnX >= 0){m_myPlayer = 1;return Update(initData);}else{m_myPlayer = 0;auto const turn = FindTurn(m_gameState);game::MakeTurn(m_gameState, turn[0], turn[1]);return { turn[0], turn[1] };}}SOutputData CBotBase::Update(SInputData turnData){game::MakeTurn(m_gameState, turnData.m_oppTurnX, turnData.m_oppTurnY);if (m_isDebugEnabled){game::SGameStateView(m_gameState).Print();}auto const turn = FindTurn(m_gameState);game::MakeTurn(m_gameState, turn[0], turn[1]);return { turn[0], turn[1] };}void CBotBase::Reset(){m_gameState = game::SGameState();m_myPlayer = -1;m_isDebugEnabled = false;}void CBotBase::SetState(game::SGameState const& gameState){m_gameState = gameState;m_myPlayer = GAME_STATE_GET_PLAYER(gameState);}}}
