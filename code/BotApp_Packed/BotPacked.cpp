...
public:struct SDebugInfo{int m_evaluatedNodesCnt;int m_visitedNodesCnt;inline void Reset(){m_evaluatedNodesCnt = 0;m_visitedNodesCnt = 0;}};
#endif 
public:CMinimaxAlgorithmBase(size_t const maxDepth, TResolver const& resolver): m_maxDepth(maxDepth), m_resolver(resolver)
...
#endif 
private:struct STraversalResult{float m_score = 0.0f;TMove m_move;};private:TResolver m_resolver;size_t m_maxDepth;
#if MIMAX_MINIMAX_ENABLE_ALPHA_BETA_PRUNING
...
#endif 
int const colorMultiplier = (depth & 1) == 0 ? 1 : -1;result.m_score = m_resolver.EvaluateState(state) * colorMultiplier;return result;}result.m_score = -std::numeric_limits<float>::max();for(auto const move: moves){TState childState = state;m_resolver.MakeMove(childState, move);
#if MIMAX_MINIMAX_ENABLE_ALPHA_BETA_PRUNING
